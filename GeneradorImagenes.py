...
def agregar_imagen_al_dataset(ruta_archivo, cantidad_por_clase=50, flip_probability=0.08, seed=None): #Se agrega la imagen al dataset únicamente si no hay una imagen igual cargada
...
# Tomar el patrón base y generar lote por cada clase
    rng = np.random.default_rng(seed)
    num_clases = 4

    for clase_deseada in range(num_clases):
        patron_base = X[clase_deseada].reshape(4, 4).astype(np.uint8)
        agregadas = 0
        intentos = 0

        while agregadas < cantidad_por_clase and intentos < (cantidad_por_clase * 20):
            intentos += 1
            ruido = rng.random((4, 4)) < flip_probability
            imagen_nueva = np.logical_xor(patron_base, ruido).astype(np.uint8)

            if not comparar_imagenes(X_datos=X, imagen_nueva=imagen_nueva):
                X.append(imagen_nueva.flatten())
                y.append(clase_deseada)
                agregadas += 1

    # Guardar el dataset completo actualizado
    ...
    print("✅ ¡Lote agregado y guardado en el archivo .npz!")
